/**
 * 数独游戏js版 Download by http://www.codefans.net
 * @author:tiger 
 * @email:0463023@163.com
 * @date:2009-3-24
 * 
 * 修改了部分代码，使其能够在OPhone上作为widget方式运行
 * @author: Ji Hao
 * @email: hao.ji@139.com
 * @date: 2009-08-13
 * 
 * @param {}
 *            size
...
Sudoku.prototype = {
	...
	// 获取指定单元格的默认颜色代码
	getDefaultColorForCell: function(row,col){
		var subSize = Math.floor(Math.sqrt(this.size));
		var r = Math.floor(row / subSize);
		var c = Math.floor(col / subSize);
		if (r % 2 == c % 2) {
			return '#000000';
		}
		else
	...
	initLayout : function() {
		var self = this;
		// 难度选择
		var level = "<input type='radio' name='level' checked>初级"
				+ "<input type='radio' name='level'>中级"
				+ "<input type='radio' name='level'>高级"
				+ "<input type='radio' name='level'>骨灰级<br/>";
		var time = '用时：<span id="timer" align="right" style="width:60;color:#37c1e1;">00:00:00</span>';
		...
		// 西部面板
		var westPanel1 = document.createElement("div");
		westPanel1.className = 'westPanel';
		westPanel1.innerHTML += '<font size="4">请选择难度：'+time+'<br/>' + level + '</font>';
		
		westPanel1.appendChild(s);
		westPanel1.appendChild(p);
		westPanel1.appendChild(reset);
		westPanel1.appendChild(finish);
		
		document.body.appendChild(westPanel1);
		...
		closeBtn.style.cssText = 'position:absolute;left:70;top:0;width:20;height:18;'
		...
		for (var i = 0; i < this.size; i++) {
			...
			numi.innerHTML = i + 1;
			numi.style.cssText = "position:absolute;text-align:'center';color: #37c1e1;font-size:20px;left:"
					+ ((i % 3)* 30 +3)
					+ "px;top:"
					+ ((Math.floor(i / 3)) * 30 + 20)
					+ "px;width:20px;height:20px;border:2px solid #666;cursor:pointer;";
		...
		// 游戏局面
		var w = (mp.clientWidth-12) / this.size;
		var h = (mp.clientHeight-12) / this.size;

		for (var i = 0; i < this.size; i++) {
			for (var j = 0; j < this.size; j++) {
				var cell = document.createElement('div');
				cell.setAttribute('id', 'cell_' + i + '_' + j);
				cell.style.cssText = "position:absolute;left:" + (j * w+j*3)
						+ "px;top:" + (i * h+i*3) + "px;width:" + w + "px;height:"
						+ h + "px;border:1px solid #666;color: #37c1e1; text-align: center;";
				
				cell.style.background = this.getDefaultColorForCell(i, j);
				mp.appendChild(cell);
			}
		}
	...
	choose : function(id) {
		...
		t.innerHTML = c;
		this.solvingStack.push([this.choosing.row, this.choosing.col, previous]);
	...
	checkAnswer : function() {
		...
		if (flag && this.gameState != 'init') {
			...
			//TODO: show a div a top layer
			// this confirm can't work on OPhone
			...
			if (c) {
				...
				this.get('timer').innerHTML = this.changeTimeToString(t + this.usedTime)+" 成功!";
			}
		}
	},
	// 判断生成的游戏局面是否只有一种答案
			...
			for (var j = 0; j < this.size; j++) {
				...
				if (rdm > checkedIndex) {
					...
					cell.onclick = function(e) {
						//TODO: not so good here
						if(this.style.borderColor=="rgb(0, 238, 0) rgb(0, 238, 0) rgb(0, 238, 0) rgb(0, 238, 0)");
						{
							//存储当前选择的待填单元格
							...
							if(self.choosing.col<6)
							{
								np.style.left = pos.x+10;
							}
							else
							{
								np.style.left = pos.x-100;
							}
							np.style.top = pos.y;
	...
	onMouseOver : function(id) {
		...
		for (var h = 0; h < this.size; h++) {
			if (h != i) {
				this.get('cell_' + h + '_' + j).style.background = '#FF66CC';// 所在列变色
			}
			if (h != j) {
				this.get('cell_' + i + '_' + h).style.background = '#FF66CC';// 所在行变色
			}
		}
		// 所在的九宫格变色
		...
		for (i = subRow * sub; i < subRow * sub + sub; i++) {
			for (j = subCol * sub; j < subCol * sub + sub; j++) {
				this.get('cell_' + i + '_' + j).style.background = '#FF66CC';
			}
		}*/
	},
	// 当鼠标移出时
	onMouseOut : function(id) {
		...
		for (var h = 0; h < this.size; h++) {
			...
			if (h != i) {
				if (sh % 2 == c % 2) {
					this.get('cell_' + h + '_' + j).style.background = '#000000';
				} else {
					this.get('cell_' + h + '_' + j).style.background = '#FFFFCC';
				}// 所在列颜色恢复
			}
			if (h != j) {
				if (sh % 2 == r % 2) {
					this.get('cell_' + i + '_' + h).style.background = '#000000';
				} else {
					this.get('cell_' + i + '_' + h).style.background = '#FFFFCC';
				}// 所在行颜色恢复
			}
		}
		// 所在的九宫格变色
		var bgColor = (c % 2 == r % 2) ? '#000000' : '#FFFFCC';
		var sub = Math.floor(Math.sqrt(this.size));
		var subRow = Math.floor(i / sub);
		var subCol = Math.floor(j / sub);
		for (i = subRow * sub; i < subRow * sub + sub; i++) {
	...
	onSelect : function(v, i, j) {
		...
		for (var h = 0; h < cp.length; h++) {
			var r = cp[h][0];
			var c = cp[h][1];
			this.get('cell_' + r + '_' + c).style.background = this.getDefaultColorForCell(r, c);
		}
		...
		for (var h = 0; h < cp.length; h++) {
			...
			if (this.solving[r * this.size + c] == v) {
				...
				//this.get('cell_' + i + '_' + j).style.background = 'red';
			}
		}
		// 如果没有冲突
		// if (flag)
	...
	reset : function() {
		...
		if (this.solvingStack.length > 0) {
			...
			//撤销的时候清除之前出现的错误提示红色标记
			var cp = this.getCheckingPosition(ss[0],ss[1]);
			//清除之前可能存在的红色标记
			for (var h = 0; h < cp.length; h++) {
				var r = cp[h][0];
				var c = cp[h][1];
